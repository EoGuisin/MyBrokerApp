...
import { ViewPropTypes, Text, View, Image, TouchableOpacity, Platform } from 'react-native';
//#endregion
...
class OpcaoMenuImage extends Component {
  constructor(props)
  {
    super(props);
  }
  ...
  render() {
    return(
      <TouchableOpacity style = {{elevation: Platform.OS === "ios" ? 0 : 5, borderRadius: 15, width: 96, height: 80, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center', marginBottom: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.8, shadowRadius: 1}} onPress = {this.props.onPress} activeOpacity = {1}>
        <Image source = {this.props.imagem} style = {{width: 20, height: 20}}/>
        <Text style = {{fontStyle: 'normal', fontWeight: 'normal', fontSize: 10, color: '#00000075'}}>{this.props.textoicon}</Text>
      </TouchableOpacity>
    );
  }
  //#endregion
...
export default connect(mapStateToProps, mapDispatchToProps)(OpcaoMenuImage);

OpcaoMenuImage.propTypes = {
