...
import { ModalEmpreendimento, ModalFormularioInicial, ModalLoading, ModalLoadingGoBack } from '../../Modais';
//#endregion
...
const KEYS_TO_FILTERS_CONFIGURACOES_FUNIL_DE_VENDAS = ['descricao']
//#endregion
...
class FunilDeVendas extends Component {
  ...
  state = {
    ...
    ConfiguracoesDoFunilDeVendas: [
      {
        id: 0,
        descricao: "Formulário inicial",
        visibilidade: () => {this.setVisibilidadeFormularioInicial(true)}
      }
    ],
    searchTermConfiguracoesDoFunilDeVendas: '',
    Empreendimento: null,
  ...
  render() {
    ...
    const filteredConfiguracoesDoFunilDeVendas = this.state.ConfiguracoesDoFunilDeVendas.filter(createFilter(this.state.searchTermConfiguracoesDoFunilDeVendas, KEYS_TO_FILTERS_CONFIGURACOES_FUNIL_DE_VENDAS))
    
    return (
      <Container style = {{ paddingBottom: 0, justifyContent: 'space-between' }}>
        <ModalFormularioInicial 
          visibilidade = {this.state.VisibilidadeModalFormularioInicial}
          colorheader = {this.props.ConfigCss[0].colorheader}
          colorbutton = {this.props.ConfigCss[0].colorbutton}
          onPressIcon = {() => {this.setVisibilidadeFormularioInicial(false)}}
          onPressConfirmar = {() => {}}
          valorSelecionado = {this.state.finalidade}
          idNome = {value => {}}
          idEmail = {value => {}}
          idTelefone = {value => {}}
          returnKeyTypeNome = {'go'}
          returnKeyTypeEmail = {'go'}
          returnKeyTypeTelefone = {Platform.OS === "ios" ? "done" : "go"}
          keyboardNome = {'default'}
          keyboardEmail = {'email-address'}
          keyboardTelefone = {'numeric'}
          onChangeNome = {() => {}}
          onChangeEmail = {() => {}}
          onChangeTelefone = {() => {}}
          valueNome = {this.state.Nome}
          valueEmail = {this.state.Email}
          valueTelefone = {this.state.TelefoneP}
          onSubmitNome = {() => {}}
          onSubmitEmail = {() => {}}
          onSubmitTelefone = {() => {}}
        />
        <ModalEmpreendimento 
          ...
          renderEmpreendimento = {this.renderConfiguracoesDoFunilDeVendas}
          filteredConfiguracoesDoFunilDeVendas = {filteredConfiguracoesDoFunilDeVendas}
          idFlatList = {(ref) => { this.FlatList = ref }}
          onChangeSearch = {(term) => {this.searchUpdateConfiguracoesDoFunilDeVendas(term)}}
          onPressVisibilidade = {() => {this.setVisibilidadeModalConfiguracoesDoFunilDeVendas(false)}}
        ...
        <View>
          <View 
            ...
            <View
              ...
              <SearchInput
                onChangeText = {(term) => {this.searchUpdateConfiguracoesDoFunilDeVendas(term)}}
                style = {{
                ...
                placeholder = 'Pesquise pela opção desejada...'
          ...
          <ScrollView ref = {(ref) => this.ScrollViewEmpresa = ref}
            ...
            <View 
              ...
              <FlatList
                ...
                data = {filteredConfiguracoesDoFunilDeVendas}
                keyExtractor = {item => String(item.id)}
                renderItem = {this.renderConfiguracoesDoFunilDeVendas}
                refreshing = {true}
              />
            </View>
          </ScrollView>
  ...
  pegandoListaDeEmpreendimentos = async () => {
    try {
      ...
      {
        this.state.ConfiguracoesDoFunilDeVendas = response
  ...
  //#region Filtrando a lista de configuracoes do funil de vendas
  searchUpdateConfiguracoesDoFunilDeVendas(term) {
    this.setState({searchTermConfiguracoesDoFunilDeVendas: term})
  }
  //#endregion

  //#region Setando a visibilidade da modal de configuracoes do funil de vendas
  setVisibilidadeModalConfiguracoesDoFunilDeVendas(value) {
  ...
  setVisibilidadeFormularioInicial = async (value) => {
    await this.setState({ VisibilidadeModalFormularioInicial: value })
  }
  //#endregion

  //#region Renderizando lista de configuracoes do funil de vendas
  renderConfiguracoesDoFunilDeVendas = ({ item }) => (
    <TouchableOpacity key = {item.id} style = {{marginHorizontal: 8}} activeOpacity = {0.5}
      onPress = {item.visibilidade}>
      <View 
        style = {{
...
export default connect(mapStateToProps, mapDispatchToProps)(FunilDeVendas);